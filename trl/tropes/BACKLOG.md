...
- `convert/time-travel-tropes` (this PR, 6) — **first chron / story-time batch (engine ≥ 0.8.1); bumped `engine_min` 0.6.0 → 0.8.1.** Vanguard + subgenre across the three Temporal Mutability models: **Fixed** — Stable Time Loop + Groundhog Day Loop; **Mutable** — Grandfather Paradox + Set Right What Once Went Wrong; **Branching** — Alternate Timeline + For Want of a Nail (all `structure/`). Each keyed on S23 story-time (`chron`/`@@`/`before`/`after`/`[+Loop]`/fork) with presentation-time as the spine. The `before`/`after` discriminators are genuine (operands in distinct `@@` scenes; verified both ways — forward 1.00, reversed/chron-less reject, per STYLE §8); Stable Time Loop keys on MUTUAL order (`before` AND `after` = a closed cycle). Needed the engine 0.8.1 fix (0.8.0 ignored story-time predicates at recognition — see the engine report). Corpus 297 → 303. (The demo narratives — time-turner, Groundhog Day, Back to the Future — are embedded as the tropes' own vignettes, superseding the standalone `examples/` drafts.)
...
- **The Stinger** — ✅ DONE (#62; re-founded on the work skeleton: `work [%Film]` + `post_credits` slot).
- **Painting the Medium** — ✅ DONE (#62; un-deferred: medium as the work's `[%Film]` essence; author
  `[&Employs]` a [+MediumArtifact]).
- **Mood Whiplash**, **Cerebus Syndrome** — ✅ DONE (the **Tone cluster**): promoted a first-class **`Tone`**
  into the prelude (`attr Tone` + poles `Levity`/`Gravity`). Mood Whiplash = the CO-PRESENCE of both poles
  linked by `[&Shatters]` (juxtaposition); Cerebus Syndrome = a tonal ARC **by position** — `opening` slot
  [+Levity], `finale` slot [+Gravity], with a `[&Darkens]` span (the work skeleton carrying the arc).

**All four demoted batch-3 tropes are now recovered.** (`![+Dead]!`-style epistemic discriminators, the work
skeleton, and the Tone promotion between them retired the "deferred" backlog.)
